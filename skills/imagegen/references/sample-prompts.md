...
These prompt recipes are for use with the `scripts/image_gen.py` CLI.
...
Execution details such as explicit CLI flags, `quality`, `input_fidelity`, masks, output formats, and local output paths are documented in `references/image-api.md`. For transparent images, prompt for a flat chroma-key background and remove it locally with `python scripts/remove_chroma_key.py`; use `gpt-image-1.5 --background transparent` for true/native transparency when the subject is too complex for chroma-key removal.

CLI model notes:
- `gpt-image-2` is the default model for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- If transparent output needs true transparency, use `gpt-image-1.5` with `--background transparent`. Note that `gpt-image-2` does not support `background=transparent`.
