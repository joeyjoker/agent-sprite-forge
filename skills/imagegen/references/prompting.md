...
These prompting principles apply to all image generation via the `scripts/image_gen.py` CLI.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in the API docs.
...
- Use `gpt-image-1.5 --background transparent --output-format png` for true/native transparency requests, failed chroma-key validation, or complex transparent subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows.

## Execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are execution controls for the CLI.
- See `references/image-api.md` for those controls.
...
- If a transparent request needs true transparency, use `gpt-image-1.5`. Note that `gpt-image-2` does not support `background=transparent`.
...
- background-extraction: For simple opaque subjects, request a clean cutout on a perfectly flat chroma-key background; crisp silhouette; generous padding; no shadows; no halos; preserve label text exactly; no restyling. Use true CLI transparency for complex subjects.
