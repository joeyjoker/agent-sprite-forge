...
This file documents the CLI and API surface for `scripts/image_gen.py`.
...
| --- | --- | --- | --- | --- |
| `gpt-image-2` | `low`, `medium`, `high`, `auto` | Always high fidelity for image inputs; do not set `input_fidelity` | `auto` or flexible sizes that satisfy the constraints below | Default for new workflows: high-quality generation and editing, text-heavy images, photorealism, compositing, identity-sensitive edits, and workflows where fewer retries matter |
...
`gpt-image-2` does not currently support the `background=transparent` parameter. The default transparent-image path is to generate with a flat chroma-key background, followed by local alpha extraction with `python scripts/remove_chroma_key.py`.

Use `gpt-image-1.5` with `background=transparent` and a transparent-capable output format such as `png` or `webp` for true/native transparency. This is appropriate when the subject is too complex for clean chroma-key removal (e.g., hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows) or when local background removal fails validation.

## Output
- `data[]` list with `b64_json` per image
- The `scripts/image_gen.py` CLI decodes `b64_json` and writes output files for you.
...
- If a request fails because a specific option is unsupported by the selected GPT Image model, retry manually without that option only when the option is not required by the user. If true transparent output is required and `gpt-image-2` does not support it, switch to `gpt-image-1.5` with `background=transparent`.
